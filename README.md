...
Waveforms must be downloaded separately from the [SXS catalog](https://www.black-holes.org/waveforms). We use the Cauchy-characteristic extracted (CCE) simulations 0001-0013 news data at Lev 5 resolution. The scripts expect `bgp_qnm_fits` to handle loading via `bgp.SXS_CCE(sxs_id, type="news", lev="Lev5", radius="R2")`. See https://arxiv.org/pdf/2510.11783 for more details on processing the waveforms.

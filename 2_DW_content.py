...
def plot_ppc_check(sxs_id, no_dw_ppc, dw_ppc, outpath):
    ...
    ax.set_title(f"SXS:BBH:{sxs_id} — posterior predictive check", fontsize=9)
...
def main():
    ...
    base_candidates = (
        [(2, 2, n, s) for n in range(N_MAX + 1) for s in parities] +
        [(3, 2, n, s) for n in range(N_MAX + 1) for s in parities] +
        [(2, 2), (3, 2)]
    )
    ...
    for sxs_id in SXS_IDS:
        ...
        save_json(
            f"{OUTPUT_DIR}/dw_{sxs_id}_no_dw.json",
        ...
        save_json(
            f"{OUTPUT_DIR}/dw_{sxs_id}_with_dw.json",
        ...
        plot_ppc_check(sxs_id, no_dw_ppc, dw_ppc, f"diagnostic_figs/ppcs/2_ppc_check_{sxs_id}.pdf")

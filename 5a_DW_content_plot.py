...
Output: figs/5a_DW_content_{SXS_ID}.png
"""
...
# Vertical geometry — two sub-bars per mode row
...
def build_layout(no_dw_modes, dw_modes):
    ...
    qnm_pro = sorted(
        [m for m in all_modes if len(m) == 4 and m[0] == 3 and m[1] == 3 and m[3] == 1],
...
def plot_content(no_dw_modes, dw_modes, t0_vals, no_dw_ppc, dw_ppc, outpath, ppc_cutoff=-5.0):
    ...
    ax.set_ylabel(r"Ringdown mode $(3, 3, n)$", labelpad=23)
...
def main():
    for sxs_id in SXS_IDS:
        no_dw_path   = f"mode_content_files/dw_{sxs_id}_33_no_dw.json"
        with_dw_path = f"mode_content_files/dw_{sxs_id}_33_with_dw.json"
        ...
        out_dir = "figs" if sxs_id == "0010" else "diagnostic_figs/mode_content"
        plot_content(no_dw_modes, dw_modes, t0_vals, no_dw_ppc, dw_ppc,
                     f"{out_dir}/5a_DW_content_{sxs_id}.png",
